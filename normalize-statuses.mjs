...
function normalizeStatus(raw) {
  ...
  const canonical = [
    ...
    'Interview', 'Offer', 'Rejected', 'Discarded', 'Deferred', 'SKIP',
  ];
  ...
  if (['evaluada', 'evaluar', 'verificar', 'condicional', 'hold', 'monitor'].includes(lower)) return { status: 'Evaluated' };
  if (['aplicado', 'aplicada', 'enviada', 'applied', 'sent'].includes(lower)) return { status: 'Applied' };
  ...
  if (['descartado', 'descartada', 'cerrada'].includes(lower)) return { status: 'Discarded' };

...
function readDeadListings() {
  // Parse the most recent apply-liveness report for DEAD/REDIRECTED rows.
  // Returns up to 8 dead listings with company, role, reason, URL.
  let files = [];
  try {
  ...
  // Look for the "Dead listings" table; pipes have | # | Company | Role | Score | Reason | URL |
  const out = [];
...
function buildOvernightActivity() {
  const today = todayString();
  const yest = yesterdayString();
  const events = [];
  for (const date of [today, yest]) {
    const f = join(ROOT, 'data', 'events', `${date}.jsonl`);
    if (!existsSync(f)) continue;
    for (const line of readFileSync(f, 'utf8').split('\n').filter(Boolean)) {
      try { events.push(JSON.parse(line)); } catch { /* skip */ }
    }
  }
  const successes = events.filter(e => e.type === 'auto_submit_success');
  const failures = events.filter(e => e.type === 'auto_submit_failed');
  const circuitOpen = events.filter(e => e.type === 'auto_submit_circuit_open');
  return { successes, failures, circuitOpen };
}

function renderOvernightSection({ successes, failures, circuitOpen }) {
  if (successes.length === 0 && failures.length === 0 && circuitOpen.length === 0) return '';

  const rows = [
    ...successes.map(e => `<tr style="border-bottom:1px solid #eee">
      <td style="padding:5px 0;width:22px"><span style="color:#22863a">✓</span></td>
      <td style="padding:5px 0;font-weight:600">${escHtml(e.company)}</td>
      <td style="padding:5px 0;color:#444;font-size:12px">${escHtml(e.role)}</td>
      <td style="padding:5px 0;font-size:11px;color:#666">${escHtml(e.ats || '')} · ${escHtml(String(e.score || ''))}/5</td>
    </tr>`),
    ...failures.map(e => `<tr style="border-bottom:1px solid #eee">
      <td style="padding:5px 0;width:22px"><span style="color:#cb2431">✗</span></td>
      <td style="padding:5px 0;font-weight:600;color:#cb2431">${escHtml(e.company)}</td>
      <td style="padding:5px 0;color:#444;font-size:12px">${escHtml(e.role)}</td>
      <td style="padding:5px 0;font-size:11px;color:#cb2431">${escHtml(e.error || 'unknown error')}</td>
    </tr>`),
  ].join('');

  const circuitAlert = circuitOpen.length > 0
    ? `<div style="background:#fff3cd;border:1px solid #ffc107;padding:8px;border-radius:4px;font-size:12px;margin-top:8px">⚠ Circuit breaker tripped ${circuitOpen.length}x — check <code>data/events/</code> and run <code>pnpm run autosubmit:dry</code></div>`
    : '';

  return `
  <h2 style="font-size:16px;margin-top:24px;margin-bottom:8px;color:#333">Overnight auto-submit — ${successes.length} submitted, ${failures.length} failed</h2>
  <table style="border-collapse:collapse;width:100%;font-size:13px">
    <thead><tr style="background:#f6f8fa">
      <th style="padding:5px;font-size:11px;color:#666;text-align:left"></th>
      <th style="padding:5px;font-size:11px;color:#666;text-align:left">COMPANY</th>
      <th style="padding:5px;font-size:11px;color:#666;text-align:left">ROLE</th>
      <th style="padding:5px;font-size:11px;color:#666;text-align:left">ATS / SCORE</th>
    </tr></thead>
    <tbody>${rows}</tbody>
  </table>
  ${circuitAlert}`;
...
function renderEmail(apps) {
  ...
  const overnight = buildOvernightActivity();
  ...
  const dead = readDeadListings();
  const deadSection = dead.length > 0
    ? `
    <h2 style="font-size:16px;margin-top:24px;margin-bottom:8px;color:#cb2431">Dead listings to discard (${dead.length})</h2>
    <table style="border-collapse:collapse;width:100%;font-size:13px">
      ${dead.map((d) => `<tr style="border-bottom:1px solid #eee">
        <td style="padding:6px 0;font-weight:600;color:#666;width:40px">#${escHtml(d.id)}</td>
        <td style="padding:6px 0">
          <div style="font-weight:600">${escHtml(d.company)}</div>
          <div style="color:#666;font-size:12px">${escHtml(d.role)}</div>
        </td>
        <td style="padding:6px 0;font-size:12px;color:#cb2431">${escHtml(d.reason)}</td>
      </tr>`).join('')}
    </table>
    <div style="font-size:12px;color:#666;margin-top:6px">→ Mark these <code>Discarded</code> in <code>data/applications.md</code> to stop them surfacing in tomorrow's digest.</div>` : '';
...
<body style="font-family:-apple-system,BlinkMacSystemFont,'Segoe UI',sans-serif;color:#1a1a1a;max-width:640px;margin:0 auto;padding:16px;background:#fff">
  ...
  ${deadSection}
  ${yesterdaySection}
  ${renderOvernightSection(overnight)}

  <div style="margin-top:24px;padding-top:12px;border-top:1px solid #eee;font-size:11px;color:#999">
    Generated by <code>scripts/daily-digest.mjs</code> · Open the dashboard for full detail · See <code>docs/RESPONSE-PLAYBOOK.md</code> for what to do on each event · Incident triage: <code>docs/INCIDENT-RUNBOOK.md</code>
  </div>
...
async function main() {
  ...
  if (DRY_RUN) {
    ...
    const previewPath = join(ROOT, 'data', 'digest-preview.html');
    try {
      const { writeFileSync } = await import('node:fs');
      writeFileSync(previewPath, html, 'utf8');
      console.log(`[daily-digest:dry] full preview written: ${previewPath}`);
    } catch (e) {

...
async function fetchJobs(q) {
  // BuiltIn.com requires browser automation — Playwright stub
  try {
    const playwright = await import('playwright');
    console.log(`[${SOURCE}] Playwright available — would navigate to builtin.com/jobs?search=${encodeURIComponent(q)}&remote=true`);
    return [];
  } catch (err) {

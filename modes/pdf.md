...
6b. **ATS pre-flight GATE (hard block):** Run `node scripts/ats-gate.mjs --cv=cv.md --jd=<jd_file> [--company=X --role=Y]`. Exit code is authoritative:
   ...
   **Single command (preferred):** `node scripts/ats-preflight.mjs --jd=<jd_file> --write-json=output/ats-score-{slug}.json [--cv=cv.md] [--company=X --role=Y] [--threshold=60]` — runs `ats-score` with `--write-json`, then `ats-gate` with `--score-file` so the gate is a fast JSON read.

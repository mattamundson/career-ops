...
3. **Extract URLs**: Read DOM results → extract list of URLs → append to `batch-input.tsv`
...
Run `batch-runner.sh --help` for full usage. After manual URL collection:
1. Add URLs to `batch/batch-input.tsv` (id, url, source, notes)
2. Run: `./batch-runner.sh --parallel 3`
3. Monitor: `tail -f batch/logs/*.log`
4. After completion: `node merge-tracker.mjs`

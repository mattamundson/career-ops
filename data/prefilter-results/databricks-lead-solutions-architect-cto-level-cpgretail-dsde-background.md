# Prefilter: Databricks — Lead Solutions Architect  (CTO level-CPG/Retail, DS/DE background)

**status:** evaluate
**auto_scored:** true
**date:** 2026-04-21
**url:** https://databricks.com/company/careers/open-positions/job?gh_jid=7803166002
**company:** Databricks
**title:** Lead Solutions Architect  (CTO level-CPG/Retail, DS/DE background)

---

## Prefilter Result

**Archetype:** Applied AI / Solutions Architect
**Quick Score:** 4/5 — auto-scored at scan time
**Top 3 Matches:**
- Lead role aligns with solutions architect
- CPG/Retail focus aligns with operational data
- Strong DS/DE background complements AI automation
**Top 3 Gaps:**
- No explicit data processing signal
- CTO-level implies less hands-on execution
- Potential overlap with executive responsibilities
**Recommendation:** EVALUATE

---

## Company Intel

[../company-intel/databricks.md](../company-intel/databricks.md)


---

## Company Intel (cached)

_Dossier file exists at `C:\Users\mattm\career-ops\data\company-intel\databricks.md` but is mostly unpopulated — treat company as unresearched._

---

## Notes

<!-- auto-scored at scan time via modes/prefilter-auto-score.md -->
